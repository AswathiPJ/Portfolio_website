...
async function basic_info_get(){
    const res = await fetch('https://api.github.com/users/AswathiPJ');
    // const res = await fetch('https://random-data-api.com/api/users/random_user');
    const data = await res.json()
    
    const basic_info_text = document.createElement('h2');
    basic_info_text.textContent="User name";
...
async function repos_info_get(){
    ...
    const res = await fetch('https://api.github.com/users/AswathiPJ/repos');
    const data = await res.json()
